...
React / Vite
   ...
   ▼
chi API ───────────────► Eino ReAct Agent ─────► 21 个模型工具
   │                         │                         │
   │                         └── TurnQueue / Hub ─────┘
   │
...
初版组装与卡点混剪也使用同一组原子编辑。空时间线先通过 `asset.list_assets` / `shot.search` 取得素材事实，再逐段 `timeline.insert`；卡点流程可并行读取 `audio.analyze_beats` 与镜头检索结果，由模型明确选择切点、镜头顺序和 source range。BGM 作为单独一次 insert 写入 `bgm` 轨，并携带检测所得的完整 `metadata.beat_grid`；可选 SFX 再单独插入和调节增益。旧 `timeline.compose_initial` 与 `timeline.recut_to_beats` 仅作为迁移期 harness 回归入口，生产模型不可见。
