...
- **Asset**（素材面板：项目详情素材 tab 与 Case 工作台共用）：**单一「本地导入」入口**——服务端文件管理器多选文件或整个文件夹（默认 reference 模式），文件夹递归导入并保留层级（面板按文件夹分组）；删除 Project 引用、禁用、失效重新定位、查看缩略图/时长/理解摘要。UI 不设上传与 URL 导入入口（分片上传 API 保留；URL 导入走对话由 agent 调 `asset.import_url`，确认卡在对话流呈现）。理解的触发与重试都在对话里由 agent 调 `understand.materials` 完成，素材面板不设手动"理解/重试"按钮（chat-first）。
...
素材面板是同一组件的两种模式：项目详情素材 tab 为**管理模式**（瓦片菜单：禁用/启用、删除引用、失效重新定位；点击瓦片展开理解摘要全文），Case 工作台右上为**试看模式**（点击素材在预览区试看）。面板为文件夹分组网格：文件夹瓦片 + 素材缩略图瓦片（时长角标、理解状态徽标 未理解/理解中/已理解/失败）、面包屑下钻。文件/文件夹选择用**服务器端目录浏览 API**（§13.4）多选——后端就是本机进程，不依赖浏览器 File System Access API；选中文件夹则递归导入其中媒体文件，相对路径存为链接的 `rel_dir`（分组依据）。理解不设任何手动按钮（含失败重试）：触发与重试都由用户在对话里表达、agent 调工具完成，UI 不加旁路入口。
...
POST /api/projects/{pid}/materials/import-local      # {path?|paths?, storage_mode?}；条目可为目录，递归导入保留层级 rel_dir，跳过项进 skipped
...
POST /api/uploads/init                 # 分片上传 API 保留；UI 无入口
...
分片上传（init/parts/complete）API 保留但 UI 不再提供入口（本地产品，本地路径 reference 导入已覆盖该场景）。目录浏览（本地导入）：
...
只读；受 §13.0 全部基线约束（token + Host/Origin + realpath canonicalize + roots allowlist）。前端目录浏览器支持**多选文件与整个文件夹** → 路径列表传回 → 后端逐条 canonicalize + allowlist 校验后导入：文件直接 reference 导入；目录递归扫描媒体文件（隐藏项跳过，不支持的扩展名记入响应 `skipped` 不中断批量），相对所选目录的子路径（含所选目录名，POSIX 分隔）写入链接的 `rel_dir`，素材面板按它分组。
...
| 素材面板 | `Materials/AssetsPanel`（文件夹分组网格，管理/试看两模式）+ `FsBrowserDialog`（多选文件/文件夹） |
