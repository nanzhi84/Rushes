...
- **Asset**（素材面板：项目详情素材 tab 与 Case 工作台共用）：**原生导入**——「＋ 导入素材」打开系统文件选择器（Finder）多选、「导入文件夹」选整个文件夹、或直接把文件/文件夹拖进面板；经分片上传以 copy 模式入库，文件夹相对路径存为 `rel_dir`（面板按文件夹分组）。删除 Project 引用、禁用、失效重新定位、查看缩略图/时长/理解摘要。UI 不设 URL 导入入口（走对话由 agent 调 `asset.import_url`，确认卡在对话流呈现）；reference 模式的本地路径导入保留为 agent 工具 / REST 能力（import-local）。理解的触发与重试都在对话里由 agent 调 `understand.materials` 完成，素材面板不设手动"理解/重试"按钮（chat-first）。
...
POST /api/projects/{pid}/materials/import-local      # agent/REST 链路（reference 模式）：{path?|paths?}，目录递归保留 rel_dir，跳过项进 skipped；UI 导入走 uploads
...
POST /api/uploads/init                 # UI 素材导入主链路（Finder 多选/文件夹/拖拽）
...
分片上传（init/parts/complete）是 **UI 素材导入的主链路**：系统原生选择器（Finder 多选 / webkitdirectory 文件夹）或拖拽（webkitGetAsEntry 递归目录）收集文件 → 逐文件分片上传 → complete 携带 `rel_dir`（文件夹相对路径，含所选目录名，POSIX 分隔）写入链接，素材面板按它分组。浏览器拿不到所选文件的磁盘路径，故 UI 导入为 copy 模式；不支持的扩展名与隐藏项在前端过滤提示。

reference 模式的本地路径导入保留为 agent 工具 / REST（import-local）：目录递归展开时对每个文件重新 realpath canonicalize + fs_roots 校验（防符号链接逃逸），越界与不支持扩展名记入响应 `skipped` 不中断批量。目录浏览 API 仅用于「重新定位失效素材」：
...
| 素材面板 | `Materials/AssetsPanel`（文件夹分组网格，管理/试看两模式；原生选择器 + 拖拽分片上传）+ `FsBrowserDialog`（仅失效重定位） |
