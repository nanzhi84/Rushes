...
**本表是"非平凡前置条件"的摘录，不是全量清单**：未列出的工具（project.create/copy、asset.import_local_file、annotation.enqueue/retry 等）前置条件仅为 `allowed_scopes` + `requires_active_project/case`，权威定义在各自 ToolSpec.requires_artifacts。`scripts/check_contracts.py` 从 ToolRegistry 生成机器可校验的全量 registry 表（49 工具 × 前置谓词 × 事件，见 §6），CI 断言文档摘录与 registry 无冲突。
