...
版本：v1.4（全量自含）
日期：2026-07-06
v1.4 变更（Spec C：agentic 素材理解）：①上传只做便宜本地索引（缩略图/分镜/波形/VAD，秒级可用，§8.1）；②离线标注与检索基建整体删除——annotation 流水线、FTS5/embedding/RRF/CandidatePack、embedding.text provider 全移除（§3.1/§3.2/§7.10/§11/§14）；③新增按需 agentic 理解：`understand.materials` 在 turn 内同步派理解子代理产出带秒级时间戳的 MaterialSummary（§4.11/§6.3/§7.4）；④选材主路径 = 主代理读摘要 + `timeline.compose_initial` 直接组装（§6.7/§10）；⑤硬约束「标注失败素材不能参与剪辑」→「理解失败可重试或经用户知情后绕过」；⑥worker 并发升为小池（§4.10）。
...
- **Asset**（只在素材管理页）：上传、本地路径导入（默认 reference 模式）、URL 导入（须确认）、删除 Project 引用、禁用、查看缩略图/时长/理解摘要。理解的触发与重试都在对话里由 agent 调 `understand.materials` 完成，素材页不设手动"理解/重试"按钮（chat-first）。
...
素材管理页展示：素材列表（缩略图、时长、理解状态徽标 未理解/理解中/已理解/失败、摘要全文可展开）、三类导入入口、删除引用。理解不设任何手动按钮（含失败重试）：触发与重试都由用户在对话里表达、agent 调工具完成，UI 不加旁路入口。本地路径导入的文件选择用**服务器端目录浏览 API**（§13.4）——后端就是本机进程，不依赖浏览器 File System Access API。

### 2.4 Case Agent Console

所有剪辑对话发生在 Case 内。包含：消息流（助手散文逐字流式 + 历史回放）、结构化交互事件、工具进度（turn 内每一步实时进入对话流成为过程条目，如"正在修改时间线…"→ ✓/✗）、预览播放器、时间线结构查看、输入框、Case 成本小计。消息流数据源为 `GET /cases/{cid}/messages`（含 user/reply/narration 三类，见 §7.9）做历史回放，实时增量经 turn-stream SSE（§2.6）推送。组件映射见 §14.4。
...
| `timeline.compose_initial` | active_case 且 **audio_plan 已确认** 且 ≥ 1 usable 未禁用 asset（基于摘要时间戳直接组装 v1，取代旧 plan_from_candidates） |
...
`compose_initial`（LLM 基于摘要给**秒级** source 段落 + role 直接组装 v1 初剪，materializer 换算成帧级 TimelineState，取代旧 plan_from_candidates）、`insert_clip`（在既有时间线按摘要时间戳补插一段 clip）、`apply_patch`（输入 TimelinePatchRequest，§7.8）、`validate`、`inspect`、`restore_version`。**LLM 不允许输出帧值、素材内部帧/时间码定位、文件路径、ffmpeg 参数**——这些字段在 LLM 可见的工具 schema 中不存在；compose_initial/insert_clip 的 source 范围以**秒（float）**表达，渲染层负责换算。用户口中的时间（秒）作为 compose_initial 的 source 段落、或 TimelinePatchRequest 中锚定 `reference` 的时间引用出现，由 Timeline Engine 解析为帧（§7.8 两阶段结构）。
...
*本文档为 v1.4 版，是本项目唯一实现依据。后续修订请递增版本号并在文首注明变更。*
