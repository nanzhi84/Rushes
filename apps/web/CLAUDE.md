...
- 结构：`src/routes/`（页面 + 同名 `.test.tsx`）、`src/components/`（`Console` / `Materials` / `PreviewPlayer` / `ProjectTree` / `TimelineViewer`）、`src/api/client.ts`（类型化 fetch 封装）、`src/auth.ts`、`src/state/`。
