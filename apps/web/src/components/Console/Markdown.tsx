...
const MarkdownBody = lazy(async () => {
  try {
    ...
    // chunk 加载失败（网络抖动 / 重新构建后旧 hash 404）：Suspense 只接管加载中、
    // 接不住 rejection，lazy 还会缓存失败——不兜底会白屏整个应用。退回纯文本渲染。
    return {
      default: function MarkdownBodyFallback({ text }: { text: string }): ReactElement {
