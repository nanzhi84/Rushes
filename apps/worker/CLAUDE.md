...
- **Job 注册表 `job_registry.py`**：`kind → async handler` 一一映射。新增一类 Job 只需在 `build_default_job_registry` 里 `registry.register(kind, handler)`。内置 kind：`noop / proxy / index / asr / tts / align / import_url / render_preview / render_final`。handler 只在 SQLite 写事务**之外**执行重活，成功返回 `JobExecutionResult`，失败抛 `JobExecutionError(retryable=...)`。
