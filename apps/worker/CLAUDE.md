...
- **Job 注册表 `job_registry.py`**：`kind → async handler` 一一映射。新增一类 Job 只需在 `build_default_job_registry` 里 `registry.register(kind, handler)`。内置 kind：`noop / poster / proxy / index / asr / tts / align / import_url / render_preview / render_final`。`poster` 是缩略图/时长秒出的快任务（claim 优先级最高，见 `packages/storage/repositories/jobs.py` 的 `CLAIM_SQL`），与 `proxy` 一同在导入时入队。handler 只在 SQLite 写事务**之外**执行重活，成功返回 `JobExecutionResult`，失败抛 `JobExecutionError(retryable=...)`。
