...
async def test_turn_queue_fifo_orders_user_message_before_observation() -> None:
    ...
    await queue.enqueue(TurnQueueItem(draft_id="draft_1", kind="user_message", item_id="message_b"))
    await queue.enqueue(
        TurnQueueItem(draft_id="draft_1", kind="job_observation", item_id="job_done")
    )
...
async def test_turn_queue_same_case_is_strictly_serial() -> None:
    ...
    await asyncio.gather(
        queue.enqueue(TurnQueueItem(draft_id="draft_1", kind="user_message", item_id="a")),
        queue.enqueue(TurnQueueItem(draft_id="draft_1", kind="ui_observation", item_id="b")),
    )
    await queue.join_all()
    await queue.shutdown()
...
async def test_turn_queue_different_cases_run_in_parallel() -> None:
    ...
    async def runner(item: TurnQueueItem, token: StopToken) -> None:
        del token
        started.append(item.draft_id)
        await release.wait()

    queue = TurnQueue(runner)
    await queue.enqueue(TurnQueueItem(draft_id="draft_1", kind="user_message"))
    await queue.enqueue(TurnQueueItem(draft_id="draft_2", kind="user_message"))

    for _ in range(20):
        if set(started) == {"draft_1", "draft_2"}:
            break
    ...
    assert set(started) == {"draft_1", "draft_2"}
...
async def test_turn_queue_stop_sets_current_turn_token_without_killing_runner() -> None:
    ...
    assert queue.request_stop("draft_1")
    release.set()
    await queue.join_all()
    await queue.shutdown()
