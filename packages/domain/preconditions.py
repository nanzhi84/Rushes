...
from contracts.draft import AudioMode, DraftState
...
class DraftArtifactStats(BaseModel):
    """Lightweight draft artifact facts needed by precondition predicates."""
...
class DraftAudioAsset(BaseModel):
    """Small immutable view of draft audio assets for Human Gate options."""
...
class PreconditionContext(BaseModel):
    ...
    draft_artifacts: DraftArtifactStats = Field(default_factory=DraftArtifactStats)
    draft_audio_assets: tuple[DraftAudioAsset, ...] = Field(default_factory=tuple)


def active_draft(context: PreconditionContext) -> bool:
    return context.draft_state is not None and context.draft_state.status == "active"


def usable_asset_exists(context: PreconditionContext) -> bool:
    # 单级草稿模型下「可用」只看「链接存在且引用有效」，不再有 enabled/disabled 维度。
    stats = context.draft_artifacts
    return bool(stats.usable_asset_ids) or stats.usable_asset_count > 0


def audio_plan_confirmed(context: PreconditionContext) -> bool:
    return context.draft_state is not None and context.draft_state.audio_plan is not None


def audio_mode_in(*modes: str | AudioMode) -> PreconditionFn:
    """Build a predicate checking DraftState.audio_plan.mode against allowed modes."""
    ...
    def predicate(context: PreconditionContext) -> bool:
        ...
        if draft_state is None or draft_state.audio_plan is None:
            return False
        return str(draft_state.audio_plan.mode) in allowed
...
def audio_source_has_audio(context: PreconditionContext) -> bool:
    ...
    if draft_state is None or draft_state.audio_plan is None:
        return False
    stats = context.draft_artifacts
    source_ids = frozenset(draft_state.audio_plan.source_asset_ids)
    if source_ids:
...
def content_plan_exists(context: PreconditionContext) -> bool:
    return context.draft_state is not None and context.draft_state.content_plan is not None


def cut_plan_exists(context: PreconditionContext) -> bool:
    return context.draft_state is not None and context.draft_state.cut_plan is not None


def timeline_exists(context: PreconditionContext) -> bool:
    return (
        context.draft_state is not None and context.draft_state.timeline_current_version is not None
    )


def timeline_validated(context: PreconditionContext) -> bool:
    return (
        context.draft_state is not None
        and context.draft_state.timeline_current_version is not None
        and context.draft_state.timeline_validated
    )


def rough_cut_approved(context: PreconditionContext) -> bool:
    return context.draft_state is not None and context.draft_state.rough_cut_approved
...
def preview_for_current_version_exists(context: PreconditionContext) -> bool:
    return (
        context.draft_state is not None
        and context.draft_state.timeline_current_version is not None
        and context.draft_state.preview_current_id is not None
    )
...
def voiceover_asset_exists(context: PreconditionContext) -> bool:
    ...
    if draft_state is None or draft_state.audio_plan is None:
        return False
    voiceover_asset_id = draft_state.audio_plan.voiceover_asset_id
    if voiceover_asset_id is None:
        return False
    return voiceover_asset_id in context.draft_artifacts.voiceover_asset_ids
