...
- **`gateway.py` 的 `ProviderGateway`** 负责选路、调用、归一化、记账（`ProviderCallRecorded`）、降级（`CapabilityDegraded`）。能力常量在 `capabilities.py`：`llm.chat / vlm.understanding / asr.transcribe / tts.speech / rerank.text`。
