"""Gateway-backed natural-language decision answer resolver.

依赖方向（PRD §15 / check_contracts）：providers 只准 import {providers, contracts}，
因此本模块不 import agent_harness——与 loop 的 DecisionAnswerResolver 协议通过
结构化鸭子类型对齐（同 providers/planner.py 先例）：loop 只读取 resolution.answer。
"""
...
class DecisionAnswerResolution:
    """结构对齐 agent_harness.decision_answering.DecisionAnswerResolution。"""
