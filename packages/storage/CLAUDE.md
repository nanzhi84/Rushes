...
- **`data_migrations.py` 是启动期幂等修复**，不是 alembic 版本迁移：`apply_data_migrations(connection)` 每次启动都跑，靠**存在性守卫**做到可重复——`_table_exists`（查 `sqlite_master`）、列检查（`PRAGMA table_info` 后判断列是否已在）后再 `ALTER`/回填/删。写新迁移遵循同样的「先检查存在与否、已是目标态就 no-op」模式。
