...
test("路径 3：Project/Case 管理、素材复用、memory 注入与移动后链接保持", async ({
  ...
  await page.getByLabel("项目名称").fill("Project B");
  ...
  await expect(page.getByRole("heading", { name: "项目级素材页" })).toBeVisible();
  await page.getByRole("button", { name: "本地路径导入" }).click();
  await page.getByRole("button", { name: new RegExp(escapeRegExp(FIXTURE_DIR)) }).click();
  await page.getByRole("button", { name: new RegExp(`^${escapeRegExp(FIXTURE_NAME)}\\b`) }).click();
  ...
  const caseRow = page.getByRole("button", { name: CASE_B_NAME }).locator("..");
  await caseRow.getByRole("button", { name: "移动" }).click();
