...
本机还需 `go`、`ffmpeg` 与 `ffprobe`。`global-setup.ts` 会重建 `.e2e-workspace`、生成确定性视频 fixture、编译两个 Go 二进制并启动：

- API：`127.0.0.1:18000`
- worker：共享 `.e2e-workspace/rushes.db`
- web：`127.0.0.1:15173`
...
- `understanding-progress-cancel.spec.ts`：理解 1/2 后协作取消，已完成摘要保留，未开始素材回到可重试状态。

失败产物保存在 `.e2e-workspace/logs`、`e2e/test-results` 和 `e2e/playwright-report`。
