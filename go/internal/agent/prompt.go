...
4. 失败后先读 observation、recovery 和已给出的最新状态，再调整参数或补取证据；不可原样重试，也不可删除用户要求来绕过错误。若结果明确业务状态未变化且已给出有效替代参数，直接只重试失败原语，不重复读取未变状态或重跑已成功原语；只有目标已陈旧、依赖新 ID/坐标或状态可能变化时才补读。恢复预算耗尽时停止调用，并如实说明已完成、未完成及下一步。
