...
func TestDynamicModelToolSurfaceUsesStateIntentAndBudgets(t *testing.T) {
	...
	assertSurface(
		"请先看看有哪些素材",
		nil,
		[]string{"asset.list_assets", "timeline.update"},
	)
	...
	assertSurface(
		...
		[]string{
			...
			"timeline.split", "preview.generate", "asset.list_assets",
		},
	)
	...
	assertSurface(
		...
		[]string{
			...
			"preview.generate", "asset.list_assets",
		},
	)
	assertSurface(
		"请组装初版时间线",
		[]string{"shot.search"},
		[]string{"asset.list_assets", "timeline.insert", "timeline.update"},
	)
	...
	assertSurface(
		"验证时间线后渲染预览",
		nil,
		[]string{"preview.generate", "preview.check", "timeline.update"},
	)
...
func TestRippleEditContinuesWithCurrentTimelineViewWithoutInspectOrCheck(t *testing.T) {
	t.Parallel()
	database := agenttest.AgentTestDatabase(t)
	...
	specs, err := selectModelToolSurface(ctx, service.tools, []*schema.Message{user, ripple})
	if err != nil {
		t.Fatal(err)
	}
	names := surfaceNames(specs)
	if containsName(names, "timeline.inspect") || containsName(names, "timeline.check") {
		t.Fatalf("Harness 内部工具泄漏到波纹编辑工具面: %v", names)
	}
	if !containsName(names, "timeline.update") {
		t.Fatalf("波纹编辑后未保留原子编辑能力: %v", names)
	}
}
...
func TestRemainingWorkflowSurfaceClassifiesEveryContinuationLane(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "preview_check", text: "检查预览有没有黑帧", want: 0},
	...
	if got := noModelToolsError(rushestools.SurfaceTimelineEdit).Error(); got !=
		"当前状态没有可绑定的模型工具: surface=8" {
...
func TestAtomicTimelineToolsAreTheOnlyEditingSurface(t *testing.T) {
	...
	if !containsName(composeNames, "shot.search") || containsName(composeNames, "asset.list_assets") ||
...
func TestTimelineEditSurfaceCanDiscoverAndInsertNewShot(t *testing.T) {
	...
	names := surfaceNames(specs)
	for _, name := range []string{"shot.search"} {
		if !containsName(names, name) {
...
func TestUnclassifiedEditLanguageConservativelyKeepsTimelineTools(t *testing.T) {
	...
	for _, prompt := range []string{"把前面三秒去掉", "把第一段缩短一点"} {
		...
		names := surfaceNames(specs)
		if !containsName(names, "timeline.update") || containsName(names, "timeline.check") ||
...
func TestDynamicModelToolSurfaceAnswersTimelineReadFromCurrentView(t *testing.T) {
	database := agenttest.AgentTestDatabase(t)
	...
	for _, prompt := range []string{"读取当前时间线", "调用 timeline.inspect 查看状态"} {
		...
		if names := surfaceNames(specs); len(names) != 0 {
			t.Errorf("%q surface=%v want no model tools", prompt, names)
		}
	}
}
...
func TestDynamicModelToolSurfaceFallsBackToPrerequisiteStage(t *testing.T) {
	...
	if names := surfaceNames(renderSpecs); len(names) != 0 {
		t.Fatalf("Preview QA 边界只由 Harness 执行，不应暴露模型工具: %v", names)
	}
	...
	for _, prompt := range []string{
		...
		if names := surfaceNames(exportSpecs); len(names) != 0 {
...
func TestSpecializedSurfaceFallsBackUntilAssetsAreUsable(t *testing.T) {
	...
	for _, prompt := range []string{
		...
		if containsName(names, "asset.list_assets") || containsName(names, "media.detect_shots") {
			t.Errorf("%q surface=%v leaked Harness-only prerequisite tools", prompt, names)
		}
	}
}
...
func TestMultipleAtomicEditsFinishBeforeRender(t *testing.T) {
	...
	specs, err = selectModelToolSurface(ctx, service.tools, append(messages,
		successfulMutationWithAutomaticCheck("timeline.update", "call_timeline_update", draftID+":v2"),
	))
	if err != nil {
		t.Fatal(err)
	}
...
func TestBroadRequestAdvancesSurfaceWhenDraftStateChanges(t *testing.T) {
	...
	if !containsName(beforeNames, "shot.search") || containsName(beforeNames, "asset.list_assets") ||
	...
	afterEditMessages := append(messages,
		successfulMutationWithAutomaticCheck("timeline.update", "call_apply", draftID+":v2"),
	)
...
func successfulMutationWithAutomaticCheck(toolName, callID, timelineID string) *schema.Message {
	return schema.ToolMessage(
		fmt.Sprintf(
			`{"status":"succeeded","data":{"timeline_id":%q,%q:{"status":"succeeded","data":{"timeline_id":%q}}}}`,
			timelineID, automaticTimelineCheckDataKey, timelineID,
		),
		callID,
...
func TestPureTimelineCheckUsesCurrentViewWithoutToolsOrEditLease(t *testing.T) {
	for _, prompt := range []string{
		"请校验当前时间线不变量和节拍对齐数据。",
		...
		t.Run(prompt, func(t *testing.T) {
			...
			if spy.calls != 1 || len(spy.bound) != 0 {
...
func TestEditThenAutomaticTimelineCheckKeepsSameTurnLease(t *testing.T) {
	database := agenttest.AgentTestDatabase(t)
	...
	if _, err := surface.Generate(ctx, []*schema.Message{
		user,
		successfulMutationWithAutomaticCheck(
			"timeline.update", "call-timeline-update", "draft-edit-then-check-same-lease:v2",
		),
	}); err != nil {
		t.Fatal(err)
	}
	if spy.calls != 2 || containsName(spy.bound[1], "timeline.check") ||
		!containsName(spy.bound[1], "timeline.update") {
...
func TestModelCannotExecuteRegisteredToolOutsideBoundSurface(t *testing.T) {
	...
	session.set([]string{"shot.search"})
	...
	if !reflect.DeepEqual(rejection.Data["available_tools"], []string{"shot.search"}) {
