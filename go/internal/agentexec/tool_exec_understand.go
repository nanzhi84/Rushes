...
func (exec *Executor) toolDetectShots(
	...
	request, err := exec.prepareDetectShotsRequest(ctx, draftID, input)
	if err != nil {
...
func (exec *Executor) prepareDetectShotsRequest(
	ctx context.Context,
	draftID string,
	input rushestools.DetectShotsInput,
) (preparedDetectShotsRequest, error) {
	assetID := strings.TrimSpace(input.AssetID)
	if assetID == "" {
		return preparedDetectShotsRequest{}, errors.New("media.detect_shots 需要一个非空 asset_id")
	}
	...
	if !found {
		return preparedDetectShotsRequest{}, fmt.Errorf("素材 %s 不属于当前草稿", assetID)
	}
	if asset.Kind != "video" {
		return preparedDetectShotsRequest{}, fmt.Errorf(
	...
	if !input.ForceRefresh {
		if _, cacheErr := storage.MaterialSummaryByFingerprint(
			...
			return preparedDetectShotsRequest{}, cacheErr
		}
	}
	return request, nil
}
...
func (exec *Executor) enqueueUnderstand(
	...
	result, err := reducer.Apply(ctx, exec.database, []contracts.Event{{
		...
		Payload: map[string]any{
			...
			"asset_id": request.Asset.ID, "idempotency_key": idempotencyKey,
			"job_payload": map[string]any{
				"asset_id": request.Asset.ID, "focus": request.Options.Focus,
				"depth": request.Options.Depth, "max_steps_per_asset": request.Options.MaxStepsPerAsset,
				"force_refresh": input.ForceRefresh, "refresh_nonce": strings.TrimSpace(input.RefreshNonce),
				"analysis_fingerprint": request.Fingerprint,
			},
	...
	if err != nil || result.Status != reducer.StatusApplied {
		if existing, found, lookupErr := exec.findUnderstandJob(ctx, idempotencyKey); lookupErr != nil {
			return rushestools.DetectShotsResult{}, errors.Join(err, lookupErr)
		} else if found {
			return exec.existingUnderstandResult(ctx, draftID, existing, request)
		}
		return rushestools.DetectShotsResult{}, errors.Join(
