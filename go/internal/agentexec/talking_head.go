...
func (exec *Executor) toolEditTalkingHead(
	ctx context.Context,
	draftID string,
	input rushestools.TalkingHeadEditInput,
	...
	asset, role, err := exec.talkingHeadAsset(ctx, draftID, selectedClip.AssetID)
	if err != nil {
		return rushestools.ToolResult{}, err
	}
	...
	if len(input.BrollAssignments) > 0 {
		shots, _, shotErr := exec.draftShotIndex(ctx, draftID, nil)
		if shotErr != nil {
...
func (exec *Executor) talkingHeadAsset(
