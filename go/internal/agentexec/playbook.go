...
Harness 会在每次成功写入后自动检查精确 timeline_id，并把完整证据附在同一工具结果；不要调用 timeline.inspect 或 timeline.check。你准备声明编辑达到可交付状态时，直接生成终态候选；Harness 会截住候选，对同版本检查已通过的精确 timeline_id 自动执行一次 preview.generate，并行完成 decode、black、freeze、silence、loudness，任务需要时再做 visual advisory，然后把一个 PreviewQAReport 回灌同一 ReAct transcript。不要调用或臆造 preview.generate、preview.check，也不要轮询 job、制造“继续”消息或触发 final export。报告暴露可由原子编辑修复的问题时继续编辑；新版本会在下一次交付边界重新验收。最终导出始终由用户在 UI 触发。`
