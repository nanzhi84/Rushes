...
func registerUnderstand(registry *Registry) error {
	return addTool[UnderstandInput, UnderstandResult](registry, "understand.materials", "幂等理解所选素材并生成可检索的逐镜头时间证据；相同素材和参数默认直接复用持久化结果，只有用户明确要求重新分析时才设置 force_refresh=true", nil, ExposureLLM, true, false)
}
...
func registerShotSearch(registry *Registry) error {
	return addTool[ShotSearchInput, ShotSearchResult](
		...
		[]string{"usable_asset_exists"}, ExposureLLM, true, false,
	)
}
...
func registerApplyPatchBatch(registry *Registry) error {
	return addTool[TimelinePatchBatchInput, ToolResult](
		...
		[]string{"timeline_exists"}, ExposureLLM, false, false,
	)
}
...
func registerBeatRecut(registry *Registry) error {
	return addTool[TimelineBeatRecutInput, ToolResult](
		...
		[]string{"usable_asset_exists"}, ExposureLLM, false, false,
	)
}
...
func registerTimelineInspect(registry *Registry) error {
	return addTool[TimelineInspectInput, ToolResult](registry, "timeline.inspect", "读取可编辑的时间线摘要与完整 track/clip ID、素材、角色和帧范围；尚无时间线时返回 timeline_exists=false，而不是失败", nil, ExposureLLM, false, false)
}
